...
- `_id` (ObjectId)
- `vin` (string, unique) – normalized VIN.
- `name` (string) – friendly label (e.g., "Family SUV").
- `acquiredAt` (ISODate) – acquisition timestamp.
- `acquiredFrom` (object) – `{ dealerName, dealerId?, address? }`.
- `initialOdometer` (number)
- `currentOdometer` (number) – last known mileage.
- `attributes` (object) – dynamic fields like trim, color, drivetrain, packages.
...
- `occurredAt` (ISODate)
- `recordedAt` (ISODate)
- `location` (object) – `{ name?, address?, geo? }`
- `type` (string) – enumerated event codes (e.g., `maintenance.oil-change`, `incident.collision`, `upgrade.audio`).
- `emoji` (string) – presentation glyph per event type.
- `summary` (string) – short headline for timeline view.
- `details` (object) – flexible payload tailored per event type.
  ...
  - `codes[]` – diagnostic/maintenance codes.
  - `notes[]` – free-form annotated entries with author + timestamp.
- `source` (object) – e.g., `{ channel: 'dealer-upload', referenceId: 'RO-12345' }`.
- `attachments[]` – metadata for stored documents (URLs, S3 keys, MIME types).
- `forecast` (object) – optional predictive maintenance data generated by chat/AI modules.
...
- **Vehicle summaries** – computed views/materialized collections storing latest valuation, next maintenance due, and recent events for fast dashboard loads.
- **Chat knowledge base** – preprocessed snippets compiled from vehicles and events for conversational context.
...
| 2018-09-06 | Electrical system service, miscellaneous fixes, lube/oil/filter, tire rotation     | Fowler Jeep of Boulder   | —        |
...
- **Flexibility vs. structure** – core fields enable consistent querying, while the `details` object supports type-specific payloads shaped by event templates.
- **Emoji mapping** – maintain a shared lookup (config document or code enum) so UI components can render consistent icons.
- **Auditability** – timestamps, `source`, and `createdBy` provide traceability for imported data and manual edits.
- **Performance** – timeline queries rely on `{ vehicleId, occurredAt }` index and optional projections to feed the UI history panel.
- **Future expansion** – multi-vehicle fleets only require additional ownership metadata and potentially a `fleetId` field on vehicles/events.
